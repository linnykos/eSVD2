...
test_that(".evaluate_objective.neg_binom works and is correct", {
  ...
  for(i in 1:n){
    for(j in 1:p){
      res1b <- res1b + (-log(-c(x_mat[i,]%*%y_mat[j,])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/(n*p)
    }
  }
  res2b <- 0
  for(i in 1:n){
    for(j in 1:p){
      res2b <- res2b + (-library_size_vec[i]*log(-c(x_mat[i,]%*%y_mat[j,])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/(n*p)
    }
  }
...
test_that(".evaluate_objective_single.exponential works and is correct in the x direction", {
  ...
  res1b <- 0
  for(j in 1:p){
    res1b <- res1b + (-log(-c(x_mat[i,]%*%y_mat[j,])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/p
  }
  res2b <- 0
  for(j in 1:p){
    res2b <- res2b + (-library_size_vec[i]*log(-c(x_mat[i,]%*%y_mat[j,])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/p
  }
...
test_that(".evaluate_objective_single.exponential works and is correct in the y direction", {
  ...
  res1b <- 0
  for(i in 1:n){
    res1b <- res1b + (-log(-c(x_mat[i,]%*%y_mat[j,])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/n
  }
  res2b <- 0
  for(i in 1:n){
    res2b <- res2b +  (-library_size_vec[i]*log(-c(x_mat[i,]%*%y_mat[j,])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/n
  }
...
test_that(".hessian_vec.exponential works and is correct in the x direction", {
  ...
  for(j in 1:p){
    ...
    res1b <- res1b + (1/c(x_mat[i,]%*%y_mat[j,])^2)*y_mat[j,] %*% t(y_mat[j,])/p
  }
  res2b <- matrix(0, k, k)
  for(j in 1:p){
    tmp <- exp(c(x_mat[i,] %*% y_mat[j,]))
    res2b <- res2b + (library_size_vec[i]/c(x_mat[i,]%*%y_mat[j,])^2)*y_mat[j,] %*% t(y_mat[j,])/p
  }
...
test_that(".hessian_vec.exponential works and is correct in the y direction", {
  ...
  for(i in 1:n){
    tmp <- exp(c(x_mat[i,] %*% y_mat[j,]))
    res1b <- res1b + (1/c(x_mat[i,]%*%y_mat[j,])^2)*x_mat[i,] %*% t(x_mat[i,])/n
  }
  res2b <- matrix(0, k, k)
  for(i in 1:n){
    tmp <- exp(c(x_mat[i,] %*% y_mat[j,]))
    res2b <- res2b + (library_size_vec[i]/c(x_mat[i,]%*%y_mat[j,])^2)*x_mat[i,] %*% t(x_mat[i,])/n
  }
