...
#   for(i in 1:n) {
#     for(j in 1:p) {
#       res1b <- res1b + (-log(x_mat[i, ] %*% y_mat[j, ]) -
#                         dat[i, j] * x_mat[i, ] %*% y_mat[j, ] +
#                         dat[i, j]^2 * (x_mat[i, ] %*% y_mat[j, ])^2 / 2) / (n * p)
#     }
#   }
#   res2b <- 0
#   for(i in 1:n) {
#     for(j in 1:p) {
#       res2b <- res2b + (-log(x_mat[i, ] %*% y_mat[j, ]) -
#                         nuisance_param_vec[j]^2 * dat[i, j] * x_mat[i, ] %*% y_mat[j, ] +
#                         nuisance_param_vec[j]^2 * dat[i, j]^2 * (x_mat[i, ] %*% y_mat[j, ])^2 / (2 * library_size_vec[i])) / (n * p)
#     }
#   }
#
...
#     res1b <- res1b + (-log(x_mat[i, ] %*% y_mat[j, ]) -
#                       dat[i, j] * x_mat[i, ] %*% y_mat[j, ] +
#                       dat[i, j]^2 * (x_mat[i, ] %*% y_mat[j, ])^2 / 2) / p
#   }
#   res2b <- 0
#   for(j in 1:p) {
#     res2b <- res2b + (-log(x_mat[i, ] %*% y_mat[j, ]) -
#                       nuisance_param_vec[j]^2 * dat[i, j] * x_mat[i, ] %*% y_mat[j, ] +
#                       nuisance_param_vec[j]^2 * dat[i, j]^2 * (x_mat[i, ] %*% y_mat[j, ])^2 / (2 * library_size_vec[i])) / p
#   }
#
#   expect_true(abs(res1 - res1b) <= 1e-4)
#   expect_true(abs(res2 - res2b) <= 1e-4)
...
#     res1b <- res1b + (-log(x_mat[i, ] %*% y_mat[j, ]) -
#                       dat[i, j] * x_mat[i, ] %*% y_mat[j, ] +
#                       dat[i, j]^2 * (x_mat[i, ] %*% y_mat[j, ])^2 / 2) / n
#   }
#   res2b <- 0
#   for(i in 1:n) {
#     res2b <- res2b + (-log(x_mat[i, ] %*% y_mat[j, ]) -
#                       nuisance_param_vec[j]^2 * dat[i, j] * x_mat[i, ] %*% y_mat[j, ] +
#                       nuisance_param_vec[j]^2 * dat[i, j]^2 * (x_mat[i, ] %*% y_mat[j, ])^2 / (2 * library_size_vec[i])) / n
#   }
#
#   expect_true(abs(res1 - res1b) <= 1e-4)
#   expect_true(abs(res2 - res2b) <= 1e-4)
...
#   res1b <- rep(0, k)
#   for(j in 1:p) {
#     res1b <- res1b + y_mat[j, ] * (-1 / c(x_mat[i, ] %*% y_mat[j, ]) - dat[i, j] +
#                                    dat[i, j]^2 * c(x_mat[i, ] %*% y_mat[j, ])) / p
#   }
#   res2b <- rep(0, k)
#   for(j in 1:p){
#     res2b <- res2b +  y_mat[j, ] * (-1 / c(x_mat[i, ] %*% y_mat[j, ]) -
#                                     nuisance_param_vec[j]^2 * dat[i, j] +
#                                     nuisance_param_vec[j]^2 * dat[i, j]^2 * c(x_mat[i, ] %*% y_mat[j, ]) / library_size_vec[i]) / p
#   }
#
...
#   res1b <- rep(0, k)
#   for(i in 1:n) {
#     res1b <- res1b + x_mat[i, ] * (-1 / c(x_mat[i, ] %*% y_mat[j, ]) - dat[i, j] +
#                                    dat[i, j]^2 * c(x_mat[i, ] %*% y_mat[j, ])) / n
#   }
#   res2b <- rep(0, k)
#   for(i in 1:n) {
#     res2b <- res2b + x_mat[i, ] * (-1 / c(x_mat[i, ] %*% y_mat[j, ]) -
#                                    nuisance_param_vec[j]^2 * dat[i, j] +
#                                    nuisance_param_vec[j]^2 * dat[i, j]^2 * c(x_mat[i, ] %*% y_mat[j, ]) / library_size_vec[i]) / n
#   }
#
#   expect_true(sum(abs(res1 - res1b)) <= 1e-4)
#   expect_true(sum(abs(res2 - res2b)) <= 1e-4)
...
#   res1b <- matrix(0, k, k)
#   for(j in 1:p) {
#     res1b <- res1b + (1 / c(x_mat[i, ] %*% y_mat[j, ])^2 +
#                       dat[i, j]^2) * tcrossprod(y_mat[j, ]) / p
#   }
#   res2b <- matrix(0, k, k)
#   for(j in 1:p) {
#     res2b <- res2b + (1 / c(x_mat[i, ] %*% y_mat[j, ])^2 +
#                       nuisance_param_vec[j]^2 * dat[i, j]^2 / library_size_vec[i]) * tcrossprod(y_mat[j, ]) / p
#   }
#
#   expect_true(sum(abs(res1 - res1b)) <= 1e-4)
#   expect_true(sum(abs(res2 - res2b)) <= 1e-4)
...
#   for(i in 1:n) {
#     res1b <- res1b + (1 / c(x_mat[i, ] %*% y_mat[j, ])^2 +
#                       dat[i, j]^2) * tcrossprod(x_mat[i,]) / n
#   }
#   res2b <- matrix(0, k, k)
#   for(i in 1:n) {
#     res2b <- res2b + (1 / c(x_mat[i, ] %*% y_mat[j, ])^2 +
#                       nuisance_param_vec[j]^2 * dat[i, j]^2 / library_size_vec[i]) * tcrossprod(x_mat[i,]) / n
#   }
#
#   expect_true(sum(abs(res1 - res1b)) <= 1e-4)
#   expect_true(sum(abs(res2 - res2b)) <= 1e-4)
