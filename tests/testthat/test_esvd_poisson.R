...
test_that(".evaluate_objective.poisson works and is correct", {
  ...
  for(i in 1:n){
    for(j in 1:p){
      res1b <- res1b + (exp(c(x_mat[i,]%*%y_mat[j,])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/(n*p)
    }
  }
  res2b <- 0
  for(i in 1:n){
    for(j in 1:p){
      res2b <- res2b + (exp(c(x_mat[i,]%*%y_mat[j,]) + log(library_size_vec[i])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/(n*p)
    }
  }
...
test_that(".evaluate_objective_single.poisson works and is correct in the x direction", {
  ...
  res1b <- 0
  for(j in 1:p){
    res1b <- res1b + (exp(c(x_mat[i,]%*%y_mat[j,])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/p
  }
  res2b <- 0
  for(j in 1:p){
    res2b <- res2b + (exp(c(x_mat[i,]%*%y_mat[j,]) + log(library_size_vec[i])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/p
  }
...
test_that(".evaluate_objective_single.poisson works and is correct in the y direction", {
  ...
  res1b <- 0
  for(i in 1:n){
    res1b <- res1b + (exp(c(x_mat[i,]%*%y_mat[j,])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/n
  }
  res2b <- 0
  for(i in 1:n){
    res2b <- res2b + (exp(c(x_mat[i,]%*%y_mat[j,]) + log(library_size_vec[i])) - dat[i,j]*c(x_mat[i,]%*%y_mat[j,]))/n
  }
...
test_that(".hessian_vec.poisson works and is correct in the x direction", {
  ...
  res1b <- matrix(0, k, k)
  for(j in 1:p){
    res1b <- res1b + (exp(c(x_mat[i,]%*%y_mat[j,])))*y_mat[j,] %*% t(y_mat[j,])/p
  }
  res2b <- matrix(0, k, k)
  for(j in 1:p){
    res2b <- res2b + (exp(c(x_mat[i,]%*%y_mat[j,])+log(library_size_vec[i])))*y_mat[j,] %*% t(y_mat[j,])/p
  }
...
test_that(".hessian_vec.poisson works and is correct in the y direction", {
  ...
  res1b <- matrix(0, k, k)
  for(i in 1:n){
    res1b <- res1b + (exp(c(x_mat[i,]%*%y_mat[j,])))*x_mat[i,] %*% t(x_mat[i,])/n
  }
  res2b <- matrix(0, k, k)
  for(i in 1:n){
    res2b <- res2b + (exp(c(x_mat[i,]%*%y_mat[j,])+log(library_size_vec[i])))*x_mat[i,] %*% t(x_mat[i,])/n
  }
