...
test_that(".fix_rank_defficiency_initialization works", {
 ...
 res <- .fix_rank_defficiency_initialization(x_mat, y_mat, domain = c(0.1, 100))
 
 expect_true(length(res) == 2)
 expect_true(all(sort(names(res)) == sort(c("x_mat", "y_mat"))))
 expect_true(sum(abs(x_mat - res$x_mat)) <= 1e-6)
 expect_true(sum(abs(y_mat - res$y_mat)) <= 1e-6)
})
...
test_that(".fix_rank_defficiency_initialization can actually fix the rank", {
 ...
 res <- .fix_rank_defficiency_initialization(x_mat, y_mat, domain = c(-100, -.1))
 
 expect_true(sum(abs(res$x_mat[,3] - x_mat[,3])) >= 1e-6)
 expect_true(sum(abs(res$y_mat[,3] - 0)) >= 1e-6)
})
...
test_that("initialize_esvd works", {
 ...
 res <- initialize_esvd(dat, family = "poisson", k = 2)
